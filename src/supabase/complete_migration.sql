-- Complete Database Migration for Train Ticket OCR System
...
-- Add triggers for updated_at (safe recreation)
...
DROP POLICY IF EXISTS "Allow public read access on passengers" ON passengers;
DROP POLICY IF EXISTS "Allow public insert on passengers" ON passengers;
DROP POLICY IF EXISTS "Allow public update on passengers" ON passengers;
DROP POLICY IF EXISTS "Allow public read access on journeys" ON journeys;
DROP POLICY IF EXISTS "Allow public insert on journeys" ON journeys;
DROP POLICY IF EXISTS "Allow public update on journeys" ON journeys;
DROP POLICY IF EXISTS "Allow public read access on passenger_profiles" ON passenger_profiles;
DROP POLICY IF EXISTS "Allow public insert on passenger_profiles" ON passenger_profiles;
DROP POLICY IF EXISTS "Allow public update on passenger_profiles" ON passenger_profiles;
...
CREATE POLICY "Allow public update on passengers" ON passengers
    FOR UPDATE USING (true) WITH CHECK (true);
...
CREATE POLICY "Allow public update on journeys" ON journeys
    FOR UPDATE USING (true) WITH CHECK (true);
...
COMMENT ON TABLE tickets IS 'Main train ticket information from IRCTC PDFs';
COMMENT ON TABLE passengers IS 'Passenger details linked to tickets';
COMMENT ON TABLE journeys IS 'Journey segments for each ticket (supports multi-segment travel)';
...
BEGIN
    ...
    RAISE NOTICE '📊 Tables created: tickets, passengers, journeys, passenger_profiles';
    RAISE NOTICE '📈 Views created: enhanced_ticket_details';
    RAISE NOTICE '🔧 Functions created: 4 utility functions for passenger management';
    RAISE NOTICE '🔒 Row Level Security enabled with public policies';
    RAISE NOTICE '🚀 System ready for train ticket processing!';
END $$;